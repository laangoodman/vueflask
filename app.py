...
# 获取会员信息
@app.route('/api/Membership/Get_members', methods=['GET', 'POST'])
def get_members():
    if request.method == 'POST':
        try:
            # 接收前端发送的数据
            ...
            # 处理数据
            # ...
            members = GetParams.Get_members(data["value"])
            print(members)
...
# 插入会员信息
@app.route('/api/Membership/Insert_members', methods=['POST'])
def insert_member():
    try:
        # 从请求中获取 JSON 数据
        data = request.json
        # 调用插入会员信息的函数
        GetParams.Insert_member(data)
        # 返回成功消息
        return jsonify({"message": "Member data inserted successfully"})
    except Exception as e:
        return jsonify({"error": str(e)})
